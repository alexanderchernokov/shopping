...
class SANITIZATION
{
    //Singleton instance of SANITIZATION
     ...
     * @param array $validators The SANITIZATION validators
     *
     * @return mixed True(boolean) or the array of error messages
     */
    public static function is_valid(array $data, array $validators)
    {
        $sanit = self::get_instance();

        $sanit->validation_rules($validators);

        if ($sanit->run($data) === false) {
            return $sanit->get_readable_errors(false);
        } else {
            return true;
        }
    }
    ...
    public static function filter_input(array $data, array $filters)
    {
        ...
        return $sanit->filter($data, $filters);
    }
     ...
     * Usage:
     *
     * SANITIZATION::set_field_names(array(
     *  "name" => "My Lovely Name",
     ...
     * @return string
     */
    public function get_readable_errors($convert_to_string = false, $field_class = 'sanit-field', $error_class = 'sanit-error-message')
    {
